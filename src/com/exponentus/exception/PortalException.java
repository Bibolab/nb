...
public class PortalException extends Exception {
	...
	private void message(String text, HttpServletResponse response, PublishAsType publishAs) {
		...
		try {
			...
			xmlText = "<?xml version = \"1.0\" encoding=\"utf-8\"?><request><error type=\"" + type + "\">" + "<message><version>"
			        + Server.serverVersion + "</version><errortext>" + XMLUtil.getAsTagValue(text) + "</errortext></message></error></request>";
			...
			if (publishAs == PublishAsType.HTML) {
				...
				// System.out.println(PortalEnv.appID+": xsl
				// transformation="+PortalEnv.errorXSL);
	...
	public static String errorMessage(Exception exception) {
		...
		try {
			...
			return "<errortext>" + message + "</errortext>".replaceAll("\"", "'") + "<stack>"
			        + addErrorMessage.replaceAll(">", "-").replaceAll("<", "-") + "</stack>\n\r";
		} catch (Exception e) {
